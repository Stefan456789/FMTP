...
public class CMDGamemode implements CommandExecutor, TabCompleter {
	...
	public boolean onCommand(CommandSender sender, Command cmd, String arg2, String[] args) {
		...
		} else sender.sendMessage("You do not have the permissions for that command!"); return true;
		...
		if (arg0.isOp() == true) {
			...
			if(args.length == 2 ) {
				...
				for(Player guess : main.getServer().getOnlinePlayers()) {
					if(guess.getDisplayName().toLowerCase().startsWith(args[1].toLowerCase())) a2.add(guess.getDisplayName().toLowerCase());
				}
